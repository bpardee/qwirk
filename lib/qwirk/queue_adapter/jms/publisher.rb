  ...
  module QueueAdapter
    class JMSPublisher
      ...
      def initialize(queue_name, topic_name, options, response_options)
        ...
        @reply_queue = nil
        if response_options
          Qwirk::JMS::Connection.session_pool.session do |session|
      ...
      def publish(marshaled_object, marshal_sym, marshal_type, props)
        message = nil
        Qwirk::JMS::Connection.session_pool.producer(@dest_options) do |session, producer|
          producer.time_to_live      = @time_to_live if @time_to_live
          producer.delivery_mode_sym = @persistent_sym
          message = Qwirk::JMS.create_message(session, marshaled_object, marshal_type)
      ...
      def with_response(message_id, &block)
        ...
        Qwirk::JMS::Connection.session_pool.consumer(options) do |session, consumer|
      ...
      class MyConsumer
        ...
        def read_response(timeout)
          ...
          message.acknowledge
          return [Qwirk::JMS.parse_response(message), message['mt:worker']]
        end
      end
    end
  end
